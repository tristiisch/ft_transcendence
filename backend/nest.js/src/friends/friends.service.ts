...
export class FriendsService {
	...
	async addFriendRequest(
		...
		if (friendshipCheck !== null) {
			throw new NotAcceptableException(
				`They is already a relation between ${user.username} and ${target.username}` +
			...
			.then(async (insertResult: InsertResult) => {
				if (insertResult.identifiers.length < 1) {
					throw new InternalServerErrorException(
						`Can't add friendship of ${friendship.user1_id} and ${friendship.user1_id}.`
					);
				...
				this.socketService.FriendRequest(
					user.id,
					target.id,
					await notif.toFront(this.userService, [user, target])
				);
	...
	async acceptFriendRequest(
		...
		if (friendship === null)
			throw new NotAcceptableException(
				`${user.username} has no friend request from ${target.username}.`
			);
	...
	async removeFriendship(
		...
		notif = await this.notifService.addNotif(notif);
		this.socketService.RemoveFriend(
			user.id,
			target.id,
			await notif.toFront(this.userService, [user, target])
		);
		...
		return await this.friendsRepository
			.delete({ id: friendship.id })
			.then((value: DeleteResult) => {
				if (!value.affected || value.affected == 0)
					throw new InternalServerErrorException(
						`Can't remove friendship of ${friendship.user1_id} and ${friendship.user2_id}.`
					);
				else
					return {
						statusCode: 200,
						message: `You are no longer friends with ${target.username}.`,
					};
			}, this.userService.lambdaDatabaseUnvailable);
	}
	...
	private async findOne(user1: number, user2: number, strict: boolean): Promise<Friendship> {
		const sqlStatement: SelectQueryBuilder<Friendship> =
			this.friendsRepository.createQueryBuilder('friendship');

		sqlStatement
			.where('friendship.user1_id = :id1', { id1: user1 })
			.andWhere('friendship.user2_id = :id2', { id2: user2 });
		if (!strict) {
			sqlStatement
				.orWhere('friendship.user1_id = :id2')
				.andWhere('friendship.user2_id = :id1');
		}
	...
	async findPendingIds(userId: number): Promise<number[]> {
		...
		sqlStatement
			.where('friendship.user1_id = :id', { id: userId })
	...
	async findWaitingIds(userId: number): Promise<number[]> {
		...
		// await this.userService.findOne(userId);
		sqlStatement
			.where('friendship.user2_id = :id', { id: userId })
			.andWhere('friendship.status = :status', { status: FriendshipStatus.PENDING });
	...
	async findAllRelations(userId: number): Promise<Friendship[]> {
		...
		sqlStatement
			.where('friendship.user1_id = :id', { id: userId })
			.orWhere('friendship.user2_id = :id');
