...
export class ChatService {
	...
	async findAllChannels(): Promise<ChatFront[]> {
		...
		const publicChannels: ChannelFront[] = await this.channelPublicRepo
			.find()
			.then(async (chs: ChannelPublic[]) => {
				const chsFront: ChannelFront[] = new Array();
				for (const ch of chs) chsFront.push(await ch.toFront(this, null, userCached));
				return chsFront;
			});
		const protectedChannels: ChannelFront[] = await this.channelProtectedRepo
			.find()
	...
	async findUserChannel(user: User, userCached: User[] | null): Promise<ChannelFront[]> {
		...
		const publicChannels: ChannelFront[] = await this.channelPublicRepo
			.findBy({ users_ids: ArrayContains([user.id]) })
			.then(async (chs: ChannelPublic[]) => {
				const chsFront: ChannelFront[] = new Array();
				for (const ch of chs) chsFront.push(await ch.toFront(this, user, userCached));
				return chsFront;
			});
		const protectedChannels: ChannelFront[] = await this.channelProtectedRepo
			.findBy({ users_ids: ArrayContains([user.id]) })
		...
		const privateChannels: ChannelFront[] = await this.channelPrivateRepo
			.findBy({ users_ids: ArrayContains([user.id]) })
	...
	async findUserDiscussion(user: User, usersCached: User[] | null): Promise<DiscussionFront[]> {
		if (!usersCached) usersCached = new Array();
		// return new Array(); // TODO remove this line
		return await this.discussionRepo
			.findBy({ users_ids: ArrayContains([user.id]) })
			.then(async (chs: Discussion[]) => {
				const chsFront: DiscussionFront[] = new Array();
				for (const ch of chs) chsFront.push(await ch.toFront(this, user, usersCached));
				return chsFront;
			});
	}
	...
	async fetchMessage(channelId: number): Promise<MessageFront[]> {
		return await this.msgRepo
			.findBy({ id_channel: channelId })
			.then(async (msgs: Message[]) => {
				const msgsFront: MessageFront[] = new Array();
				for (const msg of msgs) msgsFront.push(msg.toFront());
				return msgsFront;
			});
	}
	...
	async fetchMessageSafe(channelDTO: ChannelFetchDTO): Promise<MessageFront[]> {
		...
		if (channelDTO.password && channelDTO.type !== ChatStatus.PROTECTED)
			throw new NotAcceptableException(
				`You need to add a password to open chat type ${channelDTO.type}.`
			);
		if (channelDTO.type === ChatStatus.PROTECTED) {
			if (!channelDTO.password)
				throw new UnauthorizedException(
					`Can't use password '${channelDTO.password}' on chat type ${channelDTO.type}.`
				);
			const protectedChannel: ChannelProtected = await this.channelProtectedRepo.findOneBy({
				id: channelDTO.id,
			});
			if (protectedChannel.password !== channelDTO.password)
				throw new UnauthorizedException(
					`Wrong password for protected channel ${protectedChannel.name}.`
				);
		}
	...
	async fetchChannel(
		user: User,
		channelId: number,
		channelType: ChatStatus
	): Promise<ChannelProtected | ChannelPublic | ChannelPrivate> {
	...
	async fetchChat(
		...
		channelId: number,
		channelType: ChatStatus
	): Promise<ChannelProtected | Discussion | ChannelPublic | ChannelPrivate> {
	...
	private async findChannelAvatar(channel: Channel, @Res() res: Response) {
		...
		if (!avatar)
			throw new UnprocessableEntityException(
				`Can't get a picture from channel ${channel.name}.`
			);
		res.writeHead(200, {
	...
	async findChannelPublicAvatar(id: number, @Res() res: Response) {
		...
		if (!channel) throw new NotAcceptableException(`The public channel ${id} didn't exist.`);
	...
	async findChannelProtectedAvatar(id: number, @Res() res: Response) {
		...
		if (!channel) throw new NotAcceptableException(`The protected channel ${id} didn't exist.`);
	...
	async findChannelPrivateAvatar(id: number, @Res() res: Response) {
		...
		if (!channel) throw new NotAcceptableException(`The private channel ${id} didn't exist.`);
	...
	async createChannel(user: User, channelDTO: ChannelCreateDTO): Promise<Channel> {
		...
		if (channelDTO.avatar_64.startsWith('src/assets/')) {
			channel.avatar_64 = await toBase64(
				`${process.env.FRONT_PREFIX}://${process.env.FRONT_HOST}:${process.env.FRONT_PORT}/${channelDTO.avatar_64}`
			);
			if (!channel.avatar_64)
				throw new PreconditionFailedException(
					`Bad channel avatar '${channelDTO.avatar_64}'`
				);
		} else {
		...
		if (channel.users_ids == null) channel.users_ids = [user.id];
		else if (channel.users_ids.indexOf(user.id) == -1) channel.users_ids.push(user.id);
	...
	async addChatToDB(
		chat: ChannelPublic | ChannelProtected | ChannelPrivate | Discussion
	): Promise<Chat> {
		switch (chat.type) {
			case ChatStatus.PUBLIC:
	...
	async addDiscussion(newDiscu: Discussion): Promise<Discussion> {
		const discu: Discussion = await this.discussionRepo.findOneBy({
			users_ids: ArrayContains(newDiscu.users_ids),
		});
		if (discu)
			throw new NotAcceptableException(
				`They is already a discussion between ${newDiscu.users_ids[0]} and ${newDiscu.users_ids[1]}.`
			);
		return this.discussionRepo.save(newDiscu);
	}

	async findDiscussion(userId: number, targetId: number): Promise<Discussion> {
		return await this.discussionRepo.findOneBy({
			users_ids: ArrayContains([userId, targetId]),
		});
	}
	...
	async addMessages(
		msg: QueryDeepPartialEntity<Message> | QueryDeepPartialEntity<Message>[]
	...
	async setAdmin(
		channel: Channel,
		users_ids: number[]
	): Promise<ChannelPublic | ChannelProtected | ChannelPrivate> {
		switch (channel.type) {
			case ChatStatus.PUBLIC:
	...
	async inviteUsers(channel: ChannelPrivate, users_ids: number[]): Promise<ChannelPrivate> {
		...
		if (channel.invited_ids == null) invited_ids = new Array();
		else channel.invited_ids;
	...
	async updateProtectedChannel(channel: ChannelEditDTO): Promise<ChannelProtected> {
		...
		if (channel.name != null) dataUpdate.name = channel.name;
		if (channel.password != null) dataUpdate.password = channel.password;
	...
	async joinChannel(
		...
		} else dataUpdate.users_ids = [user.id];

		const joinMessage = async () => {
			let msg: Message = new Message();
			msg.message =
				'[DEBUG MSG CREATED BY BACK] ⚪️　' + user.username + ' just joined the channel';
			msg.id_sender = user.id;
			msg.id_channel = channel.id;
			msg = await this.addMessage(msg);
		};
	...
	async leaveChannel(
		...
		const leaveMessage = async () => {
			...
			leaveMessage.message =
				'[DEBUG MSG CREATED BY BACK] 🔴　' + user.username + ' just leaved the channel';
	...
	async saveMessageToDiscussion(user: User, targetUser: User, msg: string): Promise<Discussion> {
		let discu: Discussion = await this.discussionRepo.findOneBy({
			users_ids: ArrayContains([user.id, targetUser.id]),
		});
