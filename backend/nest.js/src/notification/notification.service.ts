...
export class NotificationService {
    ...
    public getNotif(type: NotificationType): { (notif: Notification, user: User, isAccepted: boolean): boolean } {
        if (!type)
            throw new PreconditionFailedException('Argument NotificationType is null.');
        const func = this.notifActions.get(type);
        if (!func)
            throw new NotImplementedException(`The feature of notifiction ${type.toString} is not implemented yet.`);
        return func;
    }
