...
const friendButton = computed<string>(() => {
...
function treatFriendRequest() {
	...
	UsersService.sendFriendRequest(userStore.getUsername, route.params.username as string);
...
onMounted(() => {
	if (route.params.username === userStore.getUsername) {
		...
		fetchUser(userStore.getUsername);
	} else {
			...
			<div class="flex justify-around items-center gap-2 min-h-[168px] bg-slate-900 sm:w-2/5 sm:flex-col sm:justify-evenly">
				...
				<div class="flex flex-col gap-4">
					<base-button
						...
						@click="treatFriendRequest"
						>{{ friendButton }}</base-button
					>
					<base-button
