...
function searchPlayerAvatar(senderId: number) {
	...
	if (props.inChannel)
	{
		...
		if (!avatar) avatar = (props.users.find(element => element.id === senderId))?.avatar;
	}
...
<template>
    <div v-if="messages" v-for="message in messages" :key="message.date" class="flex gap-2 w-full mt-3 mb-1.5 pl-8">
		<BaseButton link :to="redirectTo(message.idSender)">
			<img class="self-center h-8 w-8 shrink-0 rounded-full border-[1px] border-red-400 sm:self-start" :src="searchPlayerAvatar(message.idSender)">
		</BaseButton>
		<div class="flex flex-col gap-1 min-w-0">
			<div class="flex flex-col sm:items-center sm:gap-2 pt-1 text-red-300 sm:flex-row">
				<p class="text-sm ">{{ playerName(message.idSender) }}</p>
