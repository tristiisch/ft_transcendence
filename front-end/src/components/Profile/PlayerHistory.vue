       ...
       <h1 class="flex justify-center items-center w-3/4 h-[40px] sm:h-[50px] text-sm sm:text-base text-red-200 border-b border-red-500 bg-gradient-to-r from-red-500 via-red-600 to-red-500 shrink-0">MATCH HISTORY</h1>
        <div v-if="matchsHistory" class="flex flex-col items-center gap-1 sm:gap-1.5 w-full overflow-y-auto">
            <div v-for="match in matchsHistory" :key="match.date" class="flex w-full">
                <p class="w-2/5 text-right text-sm sm:text-lg" :class="colorTextScore(match)"><span class="text-xs sm:text-sm text-red-300">{{ user?.username }}</span><span class="text-red-300"> - </span><span :class="match.score[0] === -1 ? 'text-xs sm:text-sm': ''">{{ match.score[0] === -1 ? 'FORFEIT' :  match.score[0] }}</span></p>
                <p class="w-1/5 text-center grayscale"> ⚔️ </p>
                <p class="w-2/5 text-left text-xs sm:text-sm text-red-300"><span class="text-sm sm:text-lg" :class="colorTextScore(match)"><span :class="match.score[1] === -1 ? 'text-xs sm:text-sm': ''">{{ match.score[1] === -1 ? 'FORFEIT' : match.score[1] }}</span></span><span class="text-red-300"> - </span>{{ match.opponent }}</p>
            </div>
        </div>
